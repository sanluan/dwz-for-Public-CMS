...
var DWZ = {
	...
	// sbar: show sidebar
	keyCode : {
		ENTER : 13, ESC : 27, END : 35, HOME : 36, SHIFT : 16, TAB : 9, LEFT : 37, RIGHT : 39, UP : 38, DOWN : 40,
	...
	ajaxError : function(xhr, ajaxOptions, thrownError) {
		if (alertMsg ) {
			alertMsg.error("<div>Http status: " + xhr.status + " " + xhr.statusText + "</div>" + "<div>ajaxOptions: "
					+ ajaxOptions + "</div>" + "<div>thrownError: " + thrownError + "</div>");
		} else {
	...
	$.fn.extend({
		...
		layoutH : function($refBox) {
			return this.each(function() {
				...
				if ($this.isTag("table") ) {
					$this.removeAttr("layoutH").wrap('<div layoutH="' + iLayoutH + '" style="overflow:auto;height:'
							+ iH + 'px"></div>');
				} else {
		...
		inputAlert : function() {
			return this.each(function() {
				...
				function altBoxCss(opacity) {
					...
					return {
						width : $this.width(), top : position.top + 'px', left : position.left + 'px',
						opacity : opacity || 1
					};
				}
				if (getAltBox().size() < 1 ) {
					...
					var $label = $('<label class="alt" for="' + $this.attr("id") + '">' + $this.attr("alt")
 ...
 *         string consists of the following abbreviations:
 * 
 * Field | Full Form | Short Form
 * -------------+--------------------+----------------------- Year | yyyy (4
 * digits) | yy (2 digits), y (2 or 4 digits) Month | MMM (name or abbr.)| MM (2
 * digits), M (1 or 2 digits) | NNN (abbr.) | Day of Month | dd (2 digits) | d
 * (1 or 2 digits) Day of Week | EE (name) | E (abbr) Hour (1-12) | hh (2
 * digits) | h (1 or 2 digits) Hour (0-23) | HH (2 digits) | H (1 or 2 digits)
 * Hour (0-11) | KK (2 digits) | K (1 or 2 digits) Hour (1-24) | kk (2 digits) |
 * k (1 or 2 digits) Minute | mm (2 digits) | m (1 or 2 digits) Second | ss (2
 * digits) | s (1 or 2 digits) AM/PM | a |
 * 
 * NOTE THE DIFFERENCE BETWEEN MM and mm! Month=MM, not mm! Examples: "MMM d, y"
 * matches: January 01, 2000 Dec 1, 1900 Nov 20, 00 "M/d/yy" matches: 01/20/00
 * 9/2/00 "MMM dd, yyyy hh:mm:ssa" matches: "January 01, 2000 12:30:45AM"
 * ----------------------------------------------------------
 */
(function() {
	 ...
	 * 
	 * This function takes a date string and a format string. It matches If the
	 * date string matches the format string, it returns the date. If it does
...
(function($) {
	...
	$.fn.jBar = function(options) {
		var op = $.extend({
			container : "#navTab", collapse : ".collapse", toggleBut : ".toggleCollapse div", sideBar : "#sidebar",
			sideBar2 : "#sidebar_s", splitBar : "#splitBar", splitBar2 : "#splitBarProxy"
		}, options);
		return this.each(function() {
			var jbar = this;
...
(function($) {
	...
	$.rwdrag = {
		start : function(e) {
			...
			if (!$.rwdrag.current ) {
				$.rwdrag.current = {
					el : el, oleft : parseInt(el.style.left) || 0, otop : parseInt(el.style.top) || 0,
					ox : e.pageX || e.screenX, oy : e.pageY || e.screenY
				};
	...
	$
			.extend($.fn, {
				jTree : function(options) {
					var op = $.extend({
						checkFn : null, selected : "selected", exp : "expandable", coll : "collapsable",
						firstExp : "first_expandable", firstColl : "first_collapsable", lastExp : "last_expandable",
						lastColl : "last_collapsable", folderExp : "folder_expandable",
						folderColl : "folder_collapsable", endExp : "end_expandable", endColl : "end_collapsable",
						file : "file", ck : "checked", unck : "unchecked"
					}, options);
					return this
							.each(function() {
								var $this = $(this);
										...
										.each(function() {
											...
											$li
													.genTree({
														...
														options : op,
														level : 0,
														exp : (cnum > 1 ? (first ? op.firstExp : (last ? op.lastExp
																: op.exp)) : op.endExp),
														coll : (cnum > 1 ? (first ? op.firstColl : (last ? op.lastColl
																: op.coll)) : op.endColl),
														showSub : (!$this.hasClass("collapse") && ($this
				...
				subTree : function(op, level) {
					return this.each(function() {
						$(">li", this).each(function() {
							...
							$this.genTree({
								icon : op.icon, ckbox : op.ckbox, exp : isLast ? op.options.lastExp : op.options.exp,
								coll : isLast ? op.options.lastColl : op.options.coll, options : op.options,
								level : level, space : isLast ? null : op.space, showSub : op.showSub, isLast : isLast
							});
						});
					});
				},
				genTree : function(options) {
					var op = $.extend({
						icon : options.icon, ckbox : options.ckbox, exp : "", coll : "", showSub : false, level : 0,
						options : null, isLast : false
					}, options);
					return this.each(function() {
						var node = $(this);
						var tree = $(">ul", node);
						...
						if (tree.size() > 0 ) {
							...
							$(">div", node).prepend("<div class='"
									+ (op.showSub ? op.coll : op.exp)
									+ "'></div>"
									+ (op.ckbox ? "<div class='ckbox " + checked + "'></div>" : "")
									+ (op.icon ? "<div class='"
											+ (op.showSub ? op.options.folderColl : op.options.folderExp) + "'></div>"
											: ""));
							...
							$(">div>div:first,>div>a", node).click(function() {
								...
								if (!isA || tree.is(":hidden") ) {
									...
									if (op.icon ) {
										$(">div>div:last", node).toggleClass(op.options.folderExp)
												.toggleClass(op.options.folderColl);
									}
								}
							...
							$(">div", node).prepend("<div class='node'></div>"
									+ (op.ckbox ? "<div class='ckbox " + checked + "'></div>" : "")
				...
				_checkParent : function() {
					...
					$(">div>.ckbox", parent).removeClass("unchecked").removeClass("checked").removeClass(rClass)
							.addClass(aClass);
...
(function($) {
		...
		if (options.animated ) {
			if (!options.alwaysOpen && clickedActive ) {
				$.dwz.accordion.animations[options.animated]({
					toShow : jQuery([]), toHide : toHide, complete : complete, down : down,
	...
	function clickHandler(event) {
		...
		if (!event.target && !options.alwaysOpen ) {
			...
			var toHide = options.active.next() , data = {
				instance : this, options : options, newHeader : jQuery([]), oldHeader : options.active,
				newContent : jQuery([]), oldContent : toHide
		...
		data = {
			instance : this, options : options, newHeader : clicked, oldHeader : options.active, newContent : toShow,
			oldContent : toHide
	...
	$
			.extend($.dwz.accordion, {
				defaults : {
					selectedClass : "collapsable", alwaysOpen : true, animated : 'slide', event : "click",
					header : ".accordionHeader", autoheight : true, running : 0, navigationFilter : function() {
...
function initUI(_box) {
	...
	if ($("textarea.editor", $p).length > 0 ) {
		...
		$("textarea.editor", $p).each(function(i) {
			...
			if($this.attr('maxlength'))
				ueditor_loader[module][i].setOpt({ maximumWords:$this.attr('maxlength')});
	...
	$("form.required-validate", $p).each(function() {
		var $form = $(this);
		$form.validate({
			onsubmit : false, focusInvalid : false, focusCleanup : true, errorElement : "span", ignore : ".ignore",
	...
	$("a[target=navTab]", $p).each(function() {
		$(this).click(function(event) {
			...
			navTab.openTab(tabid, url, {
				title : title, fresh : fresh, external : external
			});
	...
	$("div.pagination", $p).each(function() {
		var $this = $(this);
		$this.pagination({
			targetType : $this.attr("targetType"), rel : $this.attr("rel"), totalCount : $this.attr("totalCount"),
			numPerPage : $this.attr("numPerPage"), pageNumShown : $this.attr("pageNumShown"),
...
var alertMsg = {
	...
	_open : function(type, msg, buttons) {
		...
		var boxHtml = DWZ.frag["alertBoxFrag"].replace("#type#", type).replace("#title#", this._getTitle(type))
				.replace("#message#", msg).replace("#butFragment#", butsHtml);
	...
	confirm : function(msg, options) {
		var op = {
			okName : $.regional.alertMsg.butMsg.ok, okCall : null, cancelName : $.regional.alertMsg.butMsg.cancel,
			cancelCall : null
		};
		$.extend(op, options);
		var buttons = [ {
...
var navTab = {
	...
	_currentIndex : 0,
	_op : {
		id : "navTab", stTabBox : ".navTab-tab", stPanelBox : ".navTab-panel", mainTabId : "main", close$ : "a.close",
		prevClass : "tabsLeft", nextClass : "tabsRight", stMore : ".tabsMore", stMoreLi : "ul.tabsMoreList"
	},
	...
	openTab : function(tabid, url, options) { // if found tabid replace tab,
		...
		var op = $.extend({
			title : "New Tab", data : {}, fresh : true, external : false
		}, options);
		...
		if (iOpenIndex >= 0 ) {
			...
			this._moreBox.append('<li><a href="javascript:" title="#title#">#title#</a></li>'
...
(function($) {
	$.fn.extend({
		...
		tabs : function(options) {
			var op = $.extend({
				reverse : false, eventType : "click", currentIndex : 0, stTabHeader : "> .tabsHeader",
				stTab : ">.tabsHeaderContent>ul", stTabPanel : "> .tabsContent", ajaxClass : "j-ajax",
				closeClass : "close", prevClass : "tabsLeft", nextClass : "tabsRight"
			}, options);
			return this.each(function() {
				initTab($(this));
			});
	...
	$.layerdrag = {
		start : function(obj, e, options) {
			if (!$.layerdrag.current ) {
				$.layerdrag.current = {
					el : obj, oleft : parseInt(obj.style.left) || 0, owidth : parseInt(obj.style.width) || 0,
					otop : parseInt(obj.style.top) || 0, oheight : parseInt(obj.style.height) || 0,
					ox : e.pageX || e.screenX, oy : e.pageY || e.clientY
				};
...
(function($) {
	$.pdialog = {
		_op : {
			height : 500, width : 800, minH : 40, minW : 50, total : 20, max : false, mask : false, resizable : true,
			drawable : true, maxable : true, minable : true, fresh : true
		},
		_current : null,
		_zIndex : 42,
		getCurrent : function() {
		...
		attachShadow : function(dialog) {
			...
			if (shadow.is(":hidden") )
				shadow.show();
			shadow.css({
				top : parseInt($(dialog)[0].style.top) - 2, left : parseInt($(dialog)[0].style.left) - 4,
				height : parseInt($(dialog).height()) + 8, width : parseInt($(dialog).width()) + 8,
		...
		initResize : function(resizable, dialog, target) {
			...
			resizable.css({
				top : $(dialog).css("top"), left : $(dialog).css("left"), height : $(dialog).css("height"),
		...
		maxsize : function(dialog) {
			$(dialog).data("original", {
				top : $(dialog).css("top"), left : $(dialog).css("left"), width : $(dialog).css("width"),
		...
		minimize : function(dialog) {
			...
			$(".resizable").css({
				...
				top : $(window).height() - 60, left : task.position().left, width : task.outerWidth(),
...
(function($) {
	...
	$.dialogDrag = {
		...
		start : function(dialog, event) {
			...
			var sh = $("#dialogProxy");
			sh.css({
				left : dialog.css("left"), top : dialog.css("top"), height : dialog.css("height"),
				width : dialog.css("width"), zIndex : parseInt(dialog.css("zIndex")) + 1
			}).show();
	...
	var sortDrag = {
		start : function($sortBox, $item, event, op) {
			...
			$helper.addClass('sortDragHelper').css({
				position : 'absolute', top : position.top + $sortBox.scrollTop(), left : position.left,
				zIndex : op.zIndex, width : $item.width() + 'px', height : $item.height() + 'px'
			}).jDrag({
		...
		drag : function(el, event) {
			var $helper = $(arguments[0]) , $sortBox = $helper.data('$sortBox') , $placeholder = $helper
					.data('$placeholder');
			var $items = $sortBox.find($helper.data('op')['items']).filter(':visible')
					.filter(':not(.sortDragPlaceholder, .sortDragHelper)');
		...
		_createPlaceholder : function($item) {
			return $('<' + $item[0].nodeName + ' class="sortDragPlaceholder"/>').css({
				width : $item.outerWidth() + 'px', height : $item.outerHeight() + 'px',
				marginTop : $item.css('marginTop'), marginRight : $item.css('marginRight'),
		...
		cssTable : function(options) {
			return this.each(function() {
				...
				$trs.hoverClass("hover").each(function(index) {
					...
					if($(".edit",  (!$.pdialog.getCurrent())?navTab.getCurrentPanel(): $.pdialog.getCurrent() )){
						$tr.dblclick(function() {
							$(".edit",  (!$.pdialog.getCurrent())?navTab.getCurrentPanel(): $.pdialog.getCurrent() ).click();
						});
					}
				});
				$this.find("thead [orderField]").orderBy({
					targetType : $this.attr("targetType"), rel : $this.attr("rel"), asc : $this.attr("asc") || "asc",
...
(function($) {
	...
	$.taskBar = {
		...
		_nextBut : null,
		_op : {
			id : "taskbar", taskBox : "div.taskbarContent", prevBut : ".taskbarLeft", prevDis : "taskbarLeftDisabled",
			nextBut : ".taskbarRight", nextDis : "taskbarRightDisabled", selected : "selected",
		...
		restoreDialog : function(obj) {
			...
			$(".resizable").css({
				top : $(window).height() - 60, left : $(task).position().left, height : $(task).outerHeight(),
...
function validateCallback(form, callback, confirmMsg) {
	...
	var _submitFn = function() {
		$.ajax({
			type : form.method || 'POST', url : $form.attr("action"), data : $form.serializeArray(), dataType : "json",
			cache : false, success : callback || DWZ.ajaxDone, error : DWZ.ajaxError
		});
	}
	if (confirmMsg ) {
...
function iframeCallback(form, callback) {
	...
	if ($iframe.size() == 0 ) {
		$iframe = $($.parseHTML("<iframe id='callbackframe' name='callbackframe' src='about:blank' style='display:none'></iframe>", document, true))
				.appendTo("body");
	}
	if (!form.ajax ) {
...
(function($) {
	$.fn.extend({
		pagination : function(opts) {
			var setting = {
				...
				nums$ : "li.j-num>a", jumpto$ : "li.jumpto",
			...
			return this.each(function() {
				...
				for (var i = interval.start; i < interval.end; i++) {
					pageNumFrag += setting.pageNumFrag.replaceAll("#pageNum#", i).replaceAll("#liClass#", i == pc
							.getCurrentPage() ? 'selected j-num' : 'j-num');
				}
				$this.html(DWZ.frag["pagination"].replaceAll("#pageNumFrag#", pageNumFrag)
						.replaceAll("#currentPage#", pc.getCurrentPage())).find("li").hoverClass();
		...
		orderBy : function(options) {
			...
			var op = $.extend({
				pagerForm$ : "#pagerForm", parentBox : document
			}, options);
	...
	$.extend(Pagination.prototype, {
		...
		getInterval : function() {
			...
			var start = this.getCurrentPage() > ne_half ? Math.max(Math
					.min(this.getCurrentPage() - ne_half, upper_limit), 0) : 0;
			var end = this.getCurrentPage() > ne_half ? Math.min(this.getCurrentPage() + ne_half, np) : Math
					.min(this.opts.pageNumShown, np);
			return {
				...
				lookup : function() {
					return this.each(function() {
						var $this = $(this) , options = {
							mask : true, width : $this.attr('width') || 820, height : $this.attr('height') || 500,
							maxable : eval($this.attr("maxable") || "true"),
							resizable : eval($this.attr("resizable") || "true")
						};
						$this.click(function(event) {
							_lookup = $.extend(_lookup, {
								currentGroup : $this.attr("lookupGroup") || "", suffix : $this.attr("suffix") || "",
								$target : $this, pk : $this.attr("lookupPk") || ""
							});
							var url = unescape($this.attr("href")).replaceTmById($(event.target)
				...
				multLookup : function() {
					return this.each(function() {
						...
						$this.click(function(event) {
							...
							$unitBox.find("[name='" + $this.attr("multLookup") + "']").filter(":checked")
									.each(function() {
				...
				suggest : function() {
					...
					return this.each(function() {
						...
						function _show(event) {
							...
							_lookup = $.extend(_lookup, {
								currentGroup : $input.attr("lookupGroup") || "", suffix : $input.attr("suffix") || "",
								$target : $input, pk : $input.attr("lookupPk") || ""
							});
							var url = unescape($input.attr("suggestUrl")).replaceTmById($(event.target)
									.parents(".unitBox:first"));
							if (!url.isFinishedTm() ) {
							...
							$.ajax({
								global : false, type : 'POST', dataType : "json", url : url, cache : false,
								data : postData, success : function(response) {
									if (!response )
										return;
									var html = '';
							...
							.each(function() {
								...
								$table.find("tr:first th[type]").each(function(i) {
									...
									var field = {
										...
										name : $th.attr("name") || "", defaultVal : $th.attr("defaultVal") || "",
										size : $th.attr("size") || "12", enumUrl : $th.attr("enumUrl") || "",
										lookupGroup : $th.attr("lookupGroup") || "",
										lookupUrl : $th.attr("lookupUrl") || "",
										lookupPk : $th.attr("lookupPk") || "id", suggestUrl : $th.attr("suggestUrl"),
								...
								$tbody.find("a.btnDel").click(function() {
									...
									function delDbData() {
										$.ajax({
											type : 'POST', dataType : "json", url : $btnDel.attr('href'),
								...
								if (addButTxt ) {
									var $addBut = $('<div class="button"><div class="buttonContent"><button type="button">'
					...
					function tdHtml(field) {
						...
						case 'lookup':
							...
							if (field.suggestFields ) {
								suggestFrag = 'autocomplete="off" lookupGroup="' + field.lookupGroup + '"' + suffixFrag
										+ ' suggestUrl="' + field.suggestUrl + '" suggestFields="'
										+ field.suggestFields + '"' + ' postField="' + field.postField + '"';
							}
							html = '<input type="hidden" name="' + field.lookupGroup + '.' + field.lookupPk + suffix
									+ '"/>' + '<input type="text" name="' + field.name + '"' + suggestFrag
									+ ' lookupPk="' + field.lookupPk + '" size="' + field.size + '" class="'
									+ field.fieldClass + '"/>' + '<a class="btnLook" href="' + field.lookupUrl
									+ '" lookupGroup="' + field.lookupGroup + '" ' + suggestFrag + ' lookupPk="'
									+ field.lookupPk + '" title="查找带回">查找带回</a>';
							break;
						case 'attach':
							html = '<input type="hidden" name="' + field.lookupGroup + '.' + field.lookupPk + suffix
									+ '"/>' + '<input type="text" name="' + field.name + '" size="' + field.size
									+ '" readonly="readonly" class="' + field.fieldClass + '"/>'
									+ '<a class="btnAttach" href="' + field.lookupUrl + '" lookupGroup="'
									+ field.lookupGroup + '" ' + suffixFrag + ' lookupPk="' + field.lookupPk
					...
					return this.each(function() {
						...
						$this.click(function() {
							...
							var _callback = $this.attr("callback")
									|| (targetType == "dialog" ? dialogAjaxDone : navTabAjaxDone);
...
(function($) {
	...
	$.fn.datepicker = function(opts) {
		var setting = {
			box$ : "#calendar", year$ : "#calendar [name=year]", month$ : "#calendar [name=month]",
			tmInputs$ : "#calendar .time :text", hour$ : "#calendar .time .hh", minute$ : "#calendar .time .mm",
			second$ : "#calendar .time .ss", tmBox$ : "#calendar .tm", tmUp$ : "#calendar .time .up",
			tmDown$ : "#calendar .time .down", close$ : "#calendar .close", calIcon$ : "a.inputDateButton",
		...
		return this.each(function() {
			...
			function generateCalendar(dp) {
				...
				if (dp.hasTime() ) {
					...
					var $second = $(setting.second$).val(dp.hasSecond() ? dw.second : 0).attr('step', dp.opts.ssStep)
							.focus(function() {
								changeTmMenu("ss");
							});
	...
	$.extend(Datepicker.prototype, {
		...
		getDateWrap : function(date) { // 得到年,月,日
			...
			return {
				year : y, month : m, day : date.getDate(), hour : date.getHours(), minute : date.getMinutes(),
				second : date.getSeconds(), days : days, date : date
			}
		},
		/**
		 * @param {year:2010,
		...
		jBlindUp : function(options) {
			...
			return this.each(function() {
				...
				$(this).animate({
					...
					step : function() {}, duration : op.duration, easing : op.easing, complete : function() {
						$this.css({
							display : "none"
						});
						op.call();
...
(function($) {
	...
	var _onchange = function(event) {
		...
		$.ajax({
			...
			cache : false, data : {}, success : function(json) {
				if (!json )
					return;
				var html = '';
		...
		combox : function() {
			...
			return this.each(function(i) {
				...
				var select = '<div class="combox"><div id="combox_' + cid + '" class="select"'
						+ (ref ? ' ref="' + ref + '"' : '') + '>';
				select += '<a href="javascript:" class="' + $this.attr("class") + '" name="' + name + '" value="'
						+ value + '">' + label + '</a></div></div>';
